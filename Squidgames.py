...
class SGO:
    def __init__(self, obj_function, lower_boundary, upper_boundary, iteration_number=100, dimension=2, players=50):
    ...
    def _initialize(self) -> None:
        """
        Initialize a new random positions
        """
        self.X = [np.random.randint(self.lower_boundary, self.upper_boundary,
                                    size=self.dimension) for _ in range(self.players)]
        self.X_off = self.X[:self.players//2]
        self.X_def = self.X[self.players//2:]
    ...
    def solve(self):
        ...
        for _ in range(100):
            X_off_val = []
            self.X = self.X_def+self.X_off
            for x in self.X:
                X_off_val.append(self.obj_function(x))
            BS = self.X[X_off_val.index(min(X_off_val))]
            Scc_off = []
            Scc_def = []
            r3off = random.choice(self.X_off)
            r3def = random.choice(self.X_def)
            for i in range(self.players//2):
                DG = np.mean(self.X_def, axis=0)
                self.X_off[i] = (r2*BS-r1*r3def)/2
                ws_def = self.obj_function(self.X_def[i])
                ws_off = self.obj_function(self.X_off[i])
                if ws_def <= ws_off:
                    Scc_off.append(self.X_off[i])
                    SOG = np.mean(Scc_off, axis=0)
                    self.X_off[i] = +(r1*SOG)+(BS*r2)
                else:
                    Scc_def.append(self.X_def[i])
                    OG = np.mean(self.X_def, axis=0)
                    self.X_def[i] = self.X_def[i] + BS
            o = len(Scc_off)
            p = len(Scc_def)
            for i in range(o):
                for j in range(p):
                    self.X_off[i] = (BS) - (r1 * Scc_def[j])

        print(self.obj_function(BS))
...
for i in range(1, 24):
    LB, UB, Dim, F = fitness_function(f"F{i}")
    print(f"F = {i}")
    SGO(F, LB, UB, iteration_number=1000, dimension=Dim, players=50).solve()